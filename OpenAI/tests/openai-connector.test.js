...
describe('Openai Connector', () => {
    ...
    describe('Openai Chat', () => {
        ...
        test('Chat voice message', async () => {
            const res = await chat.voiceMessage({
                inputFilePath:'./OpenAI/tests/speech/test-speech-input-en.mp3', 
                outputFilePath:'./OpenAI/tests/speech/test-speech-output-en.mp3',
    ...
    describe('Openai Files', () => {
        ...
        test('Count file tokens', async () => {
            const res = await files.countFileTokens({pathToFile:'./OpenAI/tests/fine-tune/test-fine-tune-24.jsonl'});
            // const res = await files.create({pathToFile:'./OpenAI/tests/fine-tune/test-fine-tune-24.jsonl', purpose:'fine-tune'});
        ...
        test('Creare assistant file', async () => {
            const res = await files.create({pathToFile:'./OpenAI/tests/assistants/test.csv', purpose:'assistants'});;
            expect(res).toHaveProperty('id');
            expect(res.status).toBe('processed');
    ...
    describe('Openai Fine Tune', () => {
        ...
        test('Create Fine Tune job', async () => {
            ...
            const res = await fineTune.create({pathToFile:'./OpenAI/tests/fine-tune/test-fine-tune-24.jsonl'})
            expect(res).toHaveProperty('id');
    ...
    describe('Openai Audio', () => {
        test('textToSpeech', async () => {
            const pathToFile = './OpenAI/tests/speech/test-speech-output-en.mp3';
        ...
        test('speechToText', async () => {
            const pathToFile = './OpenAI/tests/speech/test-speech-input-en.mp3';
            const res = await speech.speechToText({pathToFile});
            expect(typeof res).toBe('string');
        });
        test('Speech Translation', async () => {
            const pathToFile = './OpenAI/tests/speech/test-speech-input-ru.mp3';
    ...
    describe('Openai Images', () => {
        test('Image Generation', async () => {
            const saveAs = './OpenAI/tests/images/test-image-create-result.jpg';
            // try {
        ...
        test('Image Edit', async () => {
            const saveAs = './OpenAI/tests/images/test-edit-image-result.jpg';
            // try {
            //     await fs.promises.unlink(saveAs)
            // } catch (error) {}
        
            const res = await images.edit({text:"A futuristic landscape behind a foregraund emoticon", 
                                    pathToFile:'./OpenAI/tests/images/test-edit-image.png', 
                                    pathToMask: './OpenAI/tests/images/test-edit-image.png', 
                                    saveAs,
                                    size: "256x256"});
        ...
        test('Image Variation', async () => {
            const pathToFile = './OpenAI/tests/images/test-edit-image.png';
            const saveAs = './OpenAI/tests/images/test-image-variation-result.jpg';
            // try {
            //     await fs.promises.unlink(saveAs)
            // } catch (error) {}
    ...
    describe('Openai Recognition', () => {
        test('Image Recognition', async () => {
            const pathToFile = './OpenAI/tests/images/test-image.jpg';
        ...
        //     const pathToFile = './OpenAI/tests/videos/cat-no.mp4';
        //     const outputDir = './OpenAI/tests/videos/video-frames'
