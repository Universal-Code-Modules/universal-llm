...
const models = {
  completion: {
    ...
    'gpt-4.1': { prices: { prompt: 2.0, completion: 8.0 } },
    'gpt-4.1-mini': { prices: { prompt: 0.4, completion: 1.6 } },
    'gpt-4.1-nano': { prices: { prompt: 0.1, completion: 0.4 } },
  },
  ...
  embedding: {
    ...
    'text-embedding-ada-002': { prices: { prompt: 0.1 }, dimension: 1536 },
  },
  recognition: {
    'gpt-4-vision-preview': {
      tokens: { base: 85, block: 170 },
      prices: { prompt: 10.0, completion: 30.0 },
    },
  },
  images: {
    'dall-e-3': {
      prices: {
        standard: { '1024×1024': 0.04, '1024×1792': 0.08, '1792×1024': 0.08 },
        hd: { '1024×1024': 0.08, '1024×1792': 0.12, '1792×1024': 0.12 },
      },
    },
    'dall-e-2': {
      prices: {
        standard: { '256×256': 0.016, '512×512': 0.018, '1024×1024': 0.02 },
      },
    },
  },
  fineTune: {
    'gpt-3.5-turbo': {
      prices: { training: 8.0, prompt: 3.0, completion: 6.0 },
    },
    'davinci-002': {
      prices: { training: 6.0, prompt: 12.0, completion: 12.0 },
    },
    'babbage-002': { prices: { training: 0.4, prompt: 1.6, completion: 1.6 } },
  },
  speech: {
    'tts-1': { prices: { prompt: 15.0 } },
    'tts-1-hd': { prices: { prompt: 30.0 } },
    'whisper-1': { prices: { minute: 0.006 } },
  },
  assistants: {
    assistant: {
      prices: { 'code-interpreter-session': 0.03, 'retrieval-GB-day': 0.2 },
    },
  },
};
...
For example, you might send 20 requests with only 100 tokens to the
ChatCompletions endpoint and that would fill your limit (if your RPM was 20),
even if you did not send 150k tokens (if your TPM limit was 150k)
within those 20 requests.

Batch API queue limits are calculated based on the total number of input tokens
queued for a given model.

Tokens from pending batch jobs are counted against your queue limit.
Once a batch job is completed, its tokens are no longer counted against
that model's limit.

Other important things worth noting:

Rate limits are defined at the organization level and at the project
level, not user level.
Rate limits vary by the model being used.
Limits are also placed on the total amount an organization can spend on
the API each month. These are also known as "usage limits".
...
const tiers = {
  ...
  tier1: {
    name: 'Tier 1',
    price: 10.0,
    description: 'Paid tier with access to all models',
    limits: {
      'gpt-4-turbo': {
        RPM: 500,
        RPD: null,
        TPM: 300000,
        BATCH_QUEUE_LIMIT: 900000,
      },
  ...
  tier2: {
    name: 'Tier 2',
    price: 20.0,
    description: 'Paid tier with access to all models and additional features',
    limits: {
