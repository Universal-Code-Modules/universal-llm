...
const callWithMeasure = async (method) => {
  const begin = measureTime();
  const result = await method();
  const end = formatMeasureTime(begin);
  return { ...result, measure: end };
};
...
const callAPI = async (options = {}) => {
  ...
  const method = library[methodName].bind(library, ...params);
  try {
    if (measure) return await callWithMeasure(method);
    const result = await method();
    return result;
  } catch (error) {
...
const fileIsExist = async (file) =>
  await fs.promises.access(file).then(
    () => true,
    () => false,
  );
